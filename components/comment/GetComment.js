        ...
        <React.Fragment>
            {comments.map(comment => 
                <div key={comment.id} className="w-full h-full flex items-center mt-5">
                    <div 
                        className="bg-[#ef4444] text-white font-[Jannat] rounded-full w-[70px] h-[70px] text-center leading-[70px] select-none mx-4 text-xl">
                    ...
                    <div className="w-[50%]">
                        <span className="select-none font-[Jannat] text-[#ef4444] font-semibold">{comment?.user?.name}</span>
                        <p className="m-0">
                            {comment?.text}
                        </p>
                    </div>
